...
from home.models import Portfolio, Booking, NewsList
...
class BookClassCreateForm(forms.ModelForm):
    ...
    class Meta:
        model = Booking
        fields = ("name", "email", "phone", "abo", "day", "reduction", "comment")
        labels = {
            "name": _(""),
            "email": _(""),
            "phone": _(""),
...
class BookEventCreateForm(forms.ModelForm):
    class Meta:
        ...
        labels = {"name": _(""), "email": _(""), "phone": _(""), "comment": _("")}
        widgets = {
            "name": forms.TextInput(attrs={"placeholder": "Name"}),
            "email": forms.EmailInput(attrs={"placeholder": "Email"}),
            "phone": forms.TextInput(attrs={"placeholder": "Phone (+417600000000)"}),
            "comment": forms.Textarea(
                attrs={"placeholder": "Comments, Questions, Allergies, Injuries"}
            ),
        }
